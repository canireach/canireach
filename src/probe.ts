...
// Direct-only targets are always probed; proxy targets are only probed when a proxy is configured.
const HTTPS_DIRECT: HttpTarget[] = [
  // Domestic direct
  { label: "baidu_direct", url: "https://www.baidu.com", via: "direct" },
  { label: "taobao_direct", url: "https://www.taobao.com", via: "direct" },
  // Overseas direct — informational; partial pass is normal on restricted networks.
  ...
  // AI provider API endpoints, direct — auth-protected so any HTTP response = reachable.
...
const HTTPS_VIA_PROXY: HttpTarget[] = [
...
export type Sample = {
  ...
  proxyEgress: { ok: boolean; ip: string | null; ms: number; err?: string } | null;
...
export async function collectSample(opts: { withDownload?: boolean } = {}): Promise<Sample> {
  ...
  // Per-cycle environment discovery: which interface is primary, what's the Wi-Fi device,
  // and is there a proxy configured? These drive what probes we actually run.
  ...
  const proxyDownload = (opts.withDownload && proxyUrl)
    ? await probeProxyDownload(proxyUrl).catch((e) => ({ ok: false, bytes: 0, ms: 0, mbps: null, err: String(e) }))
    : null;
...
function emptyProxyConfig(proxyUrl: string | null): ProxyConfig {
  return {
